...
const deleteUserById = async(req, res) => {
    try {
        ...
        return res.status(200).json({message: "User Deleted Sussessfully"});
    } catch (error) {
        next(error);
    }
}
...
const deleteContactById = async(req, res) => {
    try {
        ...
        return res.status(200).json({message: "Contact Deleted Sussessfully"});
    } catch (error) {
        next(error);
    }
}

const deleteCampusInfoById = async(req,res) => {
    try {
        const id = req.params.id;
        await CampusInfo.deleteOne({_id: id});
        return res.status(200).json({message: "CampusInfo Deleted Sussessfully"});
    } catch (error) {
        next(error);
    }
}
